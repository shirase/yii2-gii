...
<?php foreach ($generator->getManyRelations() as $attribute => $relation) {
    ...
    echo "    <?= \$form->field(\$model, '$attribute')->widget(kartik\\select2\\Select2::class, ['options'=>['multiple'=>true], 'data'=>ArrayHelper::map({$relation->modelClass}::find()->all(), 'id', '{$nameField}')]) ?>\n\n";
} ?>
